...
WHERE n.nspname = 'public'
  AND p.prokind = 'f'
  AND (p.proname ILIKE %s OR p.proname = ANY(%s::text[]))
...
def _record(row):
    (oid, name, schema, identity_arguments, arguments, result, body, language,
     cost, security_definer, volatility, parallel, strict, definition) = row
    return {
        "oid": oid,
        "name": name,
        "schema": schema,
...
def fetch_selected(config, expected_names):
    with connection(config) as conn:
        with conn.cursor() as cursor:
            cursor.execute(FUNCTION_QUERY, ("%idatum%", expected_names))
...
def fetch_matching_keys(config, keys):
    if not keys:
        return {}
    records = fetch_selected(config, sorted({key.split(".", 1)[1].split("(", 1)[0] for key in keys}))
    return {key: record for key, record in records.items() if key in keys}
