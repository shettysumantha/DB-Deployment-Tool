...
Saved database selections are stored locally in `database_credentials.sqlite3`. The file contains only the alias, host, port, database name, username, and audit fields; passwords are required for every connection and are never stored. Set `CREDENTIALS_DB` to use another protected location and `APP_USER` to provide the audit identity used when saving a configuration.
...
`GET /databases`, `GET /databases/<id>`, `POST /databases/test-connection`, and `POST /databases` manage reusable connection configurations. The same routes are available under `/api`. `POST /api/connect-td` and `POST /api/test-live-connection` accept either complete connection details for a new database or `database_id` plus a password for a saved database. Other routes include `POST /api/compare`, `GET /api/functions`, `GET /api/function/<key>/diff`, `POST /api/generate-script`, `POST /api/deploy-function`, `POST /api/deploy-selected`, and `GET /api/deployment-history`.
